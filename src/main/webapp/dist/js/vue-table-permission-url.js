new Vue({
        ...
        data: function() {
            return {
                ...
                columns: [
                    ...
                    {field: 'url', title: 'Url', titleAlign: 'center',columnAlign:'center',width: 350,isEdit:true},
                    {field: 'permission_id', title: 'Permission_id', titleAlign: 'center',columnAlign:'center',width: 50,isEdit:true},
                    {field: 'page_name', title: 'PageName', titleAlign: 'center',columnAlign:'center',width: 100,isEdit:true},
				...
				selectObj: [],
				editObj: [],
				pageName: "vue-permission-url.html",
				headerInfo: []
            }
        },
        created: function(){
        ...
        methods:{
          getSourceData:function(){
        	  var self = this;
        	  fetch(VAR_GLOBAL_WEBSITE_PREFIX+"/api/permission/url/get",{
                  method: "POST",
                  headers: {
		  ...
		  addData(){
			...
			var self = this;
        	fetch(VAR_GLOBAL_WEBSITE_PREFIX+"/api/permission/url/add",{
                    method: "POST",
                    headers: {
                        'Content-Type': 'application/json;charset=UTF-8',
  					    'token': this.headerInfo[0].accesstoken,
				...
				alert("add succ");
				location.href="vue-permission-url.html";
		  },
		  modifyData(){
			for (var i=0;i<this.editObj.length;i++) {
				var self = this;
        	    fetch(VAR_GLOBAL_WEBSITE_PREFIX+"/api/permission/url/upd",{
                    method: "POST",
                    headers: {
                        'Content-Type': 'application/json;charset=UTF-8',
  					    'token': this.headerInfo[0].accesstoken,
		  ...
		  deleteData(){
			for (var i=0;i<this.selectObj.length;i++) {
        	    fetch(VAR_GLOBAL_WEBSITE_PREFIX+"/api/permission/url/del",{
                    method: "POST",
                    headers: {
                        'Content-Type': 'application/json;charset=UTF-8',
  					    'token': this.headerInfo[0].accesstoken,
