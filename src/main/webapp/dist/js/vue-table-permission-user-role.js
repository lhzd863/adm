new Vue({
        ...
        data: function() {
            return {
                ...
                columns: [
                    ...
                    {field: 'user_id', title: 'User_Id', titleAlign: 'center',columnAlign:'center',width: 350,isEdit:true},
                    {field: 'role_id', title: 'Role_Id', titleAlign: 'center',columnAlign:'center',width: 300,isEdit:true},
				...
				selectObj: [],
				editObj: [],
				pageName: "vue-permission-user-role.html",
				headerInfo: []
            }
        },
        created: function(){
        ...
        methods:{
          getSourceData:function(){
        	  var self = this;
        	  fetch(VAR_GLOBAL_WEBSITE_PREFIX+"/api/permission/userrole/get",{
                  method: "POST",
                  headers: {
		  ...
		  addData(){
			...
			var self = this;
        	fetch(VAR_GLOBAL_WEBSITE_PREFIX+"/api/permission/userrole/add",{
                    method: "POST",
                    headers: {
                        'Content-Type': 'application/json;charset=UTF-8',
  					    'token': this.headerInfo[0].accesstoken,
				...
				alert("add succ");
				location.href="vue-permission-user-role.html";
		  },
		  modifyData(){
			for (var i=0;i<this.editObj.length;i++) {
				var self = this;
        	    fetch(VAR_GLOBAL_WEBSITE_PREFIX+"/api/permission/userrole/upd",{
                    method: "POST",
                    headers: {
                        'Content-Type': 'application/json;charset=UTF-8',
  					    'token': this.headerInfo[0].accesstoken,
		  ...
		  deleteData(){
			for (var i=0;i<this.selectObj.length;i++) {
        	    fetch(VAR_GLOBAL_WEBSITE_PREFIX+"/api/permission/userrole/del",{
                    method: "POST",
                    headers: {
                        'Content-Type': 'application/json;charset=UTF-8',
  					    'token': this.headerInfo[0].accesstoken,
