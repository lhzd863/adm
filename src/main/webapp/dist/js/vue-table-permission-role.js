new Vue({
        ...
        data: function() {
            return {
                ...
                columns: [
                    ...
                    {field: 'role_name', title: 'RoleName', titleAlign: 'center',columnAlign:'center',width: 450,isEdit:true},
				...
				selectObj: [],
				editObj: [],
				pageName: "vue-permission-role.html",
				headerInfo: []
            }
        },
        created: function(){
        ...
        methods:{
          getSourceData:function(){
        	  var self = this;
        	  fetch(VAR_GLOBAL_WEBSITE_PREFIX+"/api/permission/role/get",{
                  method: "POST",
                  headers: {
		  ...
		  addData(){
			...
			var self = this;
        	fetch(VAR_GLOBAL_WEBSITE_PREFIX+"/api/permission/role/add",{
                    method: "POST",
                    headers: {
                        'Content-Type': 'application/json;charset=UTF-8',
  					    'token': this.headerInfo[0].accesstoken,
				...
				alert("add succ");
				location.href="vue-permission-role.html";
		  },
		  modifyData(){
			for (var i=0;i<this.editObj.length;i++) {
				var self = this;
        	    fetch(VAR_GLOBAL_WEBSITE_PREFIX+"/api/permission/role/upd",{
                    method: "POST",
                    headers: {
                        'Content-Type': 'application/json;charset=UTF-8',
  					    'token': this.headerInfo[0].accesstoken,
                    ...
                    body: JSON.stringify({
                        ...
                        role_name: this.editObj[i].role_name,
		  ...
		  deleteData(){
			for (var i=0;i<this.selectObj.length;i++) {
        	    fetch(VAR_GLOBAL_WEBSITE_PREFIX+"/api/permission/role/del",{
                    method: "POST",
                    headers: {
                        'Content-Type': 'application/json;charset=UTF-8',
  					    'token': this.headerInfo[0].accesstoken,
                    ...
                    body: JSON.stringify({
                        ...
                        role_name: this.selectObj[i].role_name,
