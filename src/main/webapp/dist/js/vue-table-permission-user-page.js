new Vue({
        ...
        data: function() {
            return {
                ...
                columns: [
                    ...
                    {field: 'page_id', title: 'Page_Id', titleAlign: 'center',columnAlign:'center',width: 250,isEdit:true},
                    {field: 'user_id', title: 'User_Id', titleAlign: 'center',columnAlign:'center',width: 250,isEdit:true},
				...
				selectObj: [],
				editObj: [],
				pageName: "vue-permission-user-page.html",
				headerInfo: []
            }
        },
        created: function(){
        ...
        methods:{
          getSourceData:function(){
        	  var self = this;
        	  fetch(VAR_GLOBAL_WEBSITE_PREFIX+"/api/permission/userpage/get",{
                  method: "POST",
                  headers: {
		  ...
		  addData(){
			...
			var self = this;
        	fetch(VAR_GLOBAL_WEBSITE_PREFIX+"/api/permission/userpage/add",{
                    method: "POST",
                    headers: {
                        'Content-Type': 'application/json;charset=UTF-8',
  					    'token': this.headerInfo[0].accesstoken,
				...
				alert("add succ");
				location.href="vue-permission-user-page.html";
		  },
		  modifyData(){
			for (var i=0;i<this.editObj.length;i++) {
				var self = this;
        	    fetch(VAR_GLOBAL_WEBSITE_PREFIX+"/api/permission/userpage/upd",{
                    method: "POST",
                    headers: {
                        'Content-Type': 'application/json;charset=UTF-8',
  					    'token': this.headerInfo[0].accesstoken,
		  ...
		  deleteData(){
			for (var i=0;i<this.selectObj.length;i++) {
        	    fetch(VAR_GLOBAL_WEBSITE_PREFIX+"/api/permission/userpage/del",{
                    method: "POST",
                    headers: {
                        'Content-Type': 'application/json;charset=UTF-8',
  					    'token': this.headerInfo[0].accesstoken,
